...
def evaluate_model(model, data_loader, device):
    ...
    with torch.no_grad():
        eval_progress = tqdm(data_loader, desc="Evaluating", leave=True)  # Add this line
        for inputs, labels in eval_progress:  # Changed from data_loader to eval_progress
...
def main():
    ...
    # Load data
    test_loader = get_dataloader(args.data_dir, args.batch_size, shuffle=False)
    ...
    fig = hm.get_figure()
    fig.savefig('results/VIT_FIN_HEATMAP_EVALUATION.png')
    plt.close()
