...
from data_loader_vit import get_dataloader
import timm
...
def train_model(model_name, data_dir, num_classes, num_epochs, batch_size, pretrained_path=None, learning_rate=0.001, checkpoint_path='model_checkpoints'):
    ...
    train_loader = get_dataloader(os.path.join(data_dir, 'train'), batch_size, shuffle=True)
    val_loader = get_dataloader(os.path.join(data_dir, 'val'), batch_size, shuffle=False)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            val_progress = tqdm(val_loader, desc="Validation", leave=True)  # Add this line
            for inputs, labels in val_progress:  # Changed from val_loader to val_progress
                inputs, labels = inputs.to(device), labels.to(device)
                outputs = model(inputs)
                loss = criterion(outputs, labels)
                val_loss += loss.item()
    ...
    x_ala = ['Cancer', 'No_Cancer'] # labels for x-axis
    y_ala = ['Cancer', 'No_Cancer'] # labels for y-axis
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='vit_base_patch16_224', help='Timm model name')
    parser.add_argument('--data_dir', required=True, help='Dataset path')
    parser.add_argument('--num_classes', type=int, default=2)
    parser.add_argument('--num_epochs', type=int, default=50)
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--learning_rate', type=float, default=0.001)
    parser.add_argument('--checkpoint_path', default='vit_checkpoints')
